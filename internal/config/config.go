...
type Config struct {
	...
	Path              string            `toml:"-"`
	NextcloudUsername string            `toml:"username"`
	NextcloudPassword string            `toml:"password"`
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("NEXTCLOUD_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "nextcloud-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("NEXTCLOUD_USERNAME"); v != "" {
		cfg.NextcloudUsername = v
		cfg.AuthSource = "env:NEXTCLOUD_USERNAME"
	}
	if v := os.Getenv("NEXTCLOUD_PASSWORD"); v != "" {
		cfg.NextcloudPassword = v
		cfg.AuthSource = "env:NEXTCLOUD_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.NextcloudUsername != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.NextcloudPassword != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("NEXTCLOUD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("NEXTCLOUD_BASE_PATH"); v != "" {
		cfg.BasePath = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("NEXTCLOUD_HOST")); v != "" {
		cfg.TemplateVars["host"] = normalizeEndpointTemplateValue(v)
	} else {
		cfg.TemplateVars["host"] = "http://localhost"
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.NextcloudUsername == "" || c.NextcloudPassword == "" {
		return ""
	}
	credentials := c.NextcloudUsername + ":" + c.NextcloudPassword
