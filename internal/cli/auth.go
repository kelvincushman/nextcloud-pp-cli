...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Nextcloud",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  nextcloud-pp-cli auth setup\n  nextcloud-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export NEXTCLOUD_USERNAME=\"<your-token>\"")
			fmt.Fprintln(w, "  export NEXTCLOUD_PASSWORD=\"<your-token>\"")
			fmt.Fprintln(w, "  nextcloud-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nextcloud-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export NEXTCLOUD_USERNAME=\"your-token-here\"")
				fmt.Fprintln(w, "  export NEXTCLOUD_PASSWORD=\"your-token-here\"")
				fmt.Fprintf(w, "  nextcloud-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nextcloud-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nextcloud-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("NEXTCLOUD_USERNAME") != "" {
				envStillSet = "NEXTCLOUD_USERNAME"
			}
			if envStillSet == "" && os.Getenv("NEXTCLOUD_PASSWORD") != "" {
				envStillSet = "NEXTCLOUD_PASSWORD"
			}
