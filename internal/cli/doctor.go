...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nextcloud-pp-cli doctor
  nextcloud-pp-cli doctor --json
  nextcloud-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export NEXTCLOUD_USERNAME=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("NEXTCLOUD_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "NEXTCLOUD_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "NEXTCLOUD_USERNAME")
			}
			if os.Getenv("NEXTCLOUD_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "NEXTCLOUD_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "NEXTCLOUD_PASSWORD")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'nextcloud-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'nextcloud-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'nextcloud-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'nextcloud-pp-cli sync' to refresh."
	}
	return report
}
