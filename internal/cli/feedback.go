...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".nextcloud-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NEXTCLOUD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NEXTCLOUD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nextcloud-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.nextcloud-pp-cli/feedback.jsonl.
When ` + "`NEXTCLOUD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NEXTCLOUD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nextcloud-pp-cli feedback list
  nextcloud-pp-cli feedback list --limit 5
  nextcloud-pp-cli feedback list --json`,
