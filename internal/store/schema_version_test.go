...
// TestMigrate_AddsColumnsOnUpgrade_IndexPhp verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_IndexPhp(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "index_php" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("index_php")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from index_php after migrate", want)
		}
	}
}
