# Nextcloud Printed CLI Agent Guide

This directory is a generated `nextcloud-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nextcloud-pp-cli which "<capability>" --json
nextcloud-pp-cli <command> --help
```
...
```bash
nextcloud-pp-cli <command> --agent
```
...
```bash
nextcloud-pp-cli <command> --help
nextcloud-pp-cli <command> --dry-run --agent
```
